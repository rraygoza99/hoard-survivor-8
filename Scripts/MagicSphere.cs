...
public partial class MagicSphere : Area3D
{
	...
	private float _magicSphereDamage = 0.0f;
	private Node3D _caster = null;

	public void SetPlayerStats(float criticalChance, float criticalDamageMultiplier, float lifeSteal, float generalDamage, float magicSphereDamage, Node3D caster)
	{
		_criticalChance = criticalChance;
		_criticalDamageMultiplier = criticalDamageMultiplier;
		_lifeSteal = lifeSteal;
		_generalDamage = generalDamage;
		_magicSphereDamage = magicSphereDamage;
		_caster = caster;
	}
	...
	private float CalculateDamage()
	{
		...
		float damageMultiplier = 1.0f + (_generalDamage / 100.0f) + (_magicSphereDamage / 100.0f);
		...
		if (isCritical)
		{
			...
			GD.Print($"Magic Sphere CRITICAL HIT! Damage: {finalDamage:F1} (base: {baseDamage:F1}, multiplier: {damageMultiplier:F2}x, crit: {_criticalDamageMultiplier:F1}x)");
		}
